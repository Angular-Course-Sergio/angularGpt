...
export class GptMessageEditableImageComponent implements AfterViewInit {
  ...
  onMouseMove(event: MouseEvent) {
    ...
    const curretX = event.clientX - canvasRef.getBoundingClientRect().left;
    const curretY = event.clientY - canvasRef.getBoundingClientRect().top;

    const width = curretX - this.coords().x;
    const height = curretY - this.coords().y;
